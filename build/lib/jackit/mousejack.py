...
class MouseJack:
    ...
    def __init__(self, disable_lna=False, debug=False, reset=False):
        self.channels = range(2, 84)  # Use `range` for Python 3
    ...
    def add_device(self, address, payload):
        ...
        if address in self.devices:
            ...
            self.devices[address] = {
                ...
                'count': 1,
                'timestamp': time.time(),
                'channels': [self.channels[self.channel_index]],
                'address': self.from_display(address)[::-1],
                'device': self.get_hid(payload),
                'payload': payload
            }
    ...
    def scan(self, timeout=5.0, callback=None):
        ...
        while time.time() - start_time < timeout:
            ...
            if len(value) >= 5:
                ...
                self._debug("ch: %02d addr: %s packet: %s" % 
                            (self.channels[self.channel_index], 
                             self.to_display(address), 
                             self.to_display(payload)))
                if callback:
